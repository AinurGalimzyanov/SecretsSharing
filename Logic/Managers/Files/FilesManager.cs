...
public class FilesManager  : BaseManager<FilesDal, Guid>, IFilesManager
{
    ...
    public FilesManager(IFilesRepository repository, UserManager<UserDal> userManager, IConfiguration configuration)
        : base(repository)
    {
    ...
    {
        var api = new DiskHttpApi(_configuration["YandexDisk:oauthKey"]);
    ...
    public async Task<Tuple<Stream, string, string>> DownloadFileAsync(FilesDal dal)
    {
        ...
        return new Tuple<Stream, string, string>(fileStream, fileType, dal.Name);
    }
